// src/pages/admin/SuperAdminUserManagementPage.jsx
import {
  Add as AddIcon,
...
const SuperAdminUserManagementPage = () => {
  ...
  const roleOptions = [
    ...
    { value: 'ROLE_SUPER_USER', label: 'Super Utilisateur' }
  ];
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      // Filtrer pour garder uniquement les admin et super-user
  ...
  const handleOpenEditDialog = (user) => {
    ...
    setFormData({
      ...
      email: user.email || '',
      password: '',
      role: user.roles.find(role => role === 'ROLE_ADMIN' || role === 'ROLE_SUPER_USER' || 
                          role.name === 'ROLE_ADMIN' || role.name === 'ROLE_SUPER_USER') || 'ROLE_SUPER_USER',
  ...
  const getRoleIcon = (role) => {
    ...
    switch (role) {
      ...
      case 'ROLE_SUPER_USER':
        return <SuperUserIcon fontSize="small" />;
      default:
                  ...
                  <TableRow key={user.id}>
                    ...
                    <TableCell>
                      {Array.isArray(user.roles) && user.roles.some(role => 
                        role === 'ROLE_ADMIN' || role === 'ROLE_SUPER_USER' || 
                        (typeof role === 'object' && (role.name === 'ROLE_ADMIN' || role.name === 'ROLE_SUPER_USER'))
                      ) && (
                        <Chip
